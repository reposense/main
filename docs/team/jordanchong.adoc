...
*** Re-arrange features in UserGuide into appropriate categories (Fundamentals, Player, Team, User Experience, Coming Soon). [https://github.com/CS2103JAN2018-F14-B1/main/pull/110[PR#110]]
*** Update User Guide on all Team-Related Commands, `setmatch`, `nextmatch`, `remark`, `help`, `undo`, `redo`, and `select`.
*** Wrote the preface for the Features segment, "Getting started with Commands", "The Use Of Alias", and "Lookout for Tips, Notes, and Expected Outcomes".
*** Create Command Summary Table in User Guide for ease of viewing and quick access to commands.
*** Introduce the "Overview" segment into the development guide.
*** Wrote the sections on "Remark a Player", "Team-Related Enhancements", and "Optional Field Add Command".
** Community:
