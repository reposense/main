...
+
.  Your new player, `Ronaldo` should appear in the player list.
...
. Create your first team, `Arsenal` by typing the command `create Arsenal` in the command box and pressing kbd:[Enter] to execute it.
+
image::Create.png[width="790"]
+
. Your new team, `Arsenal` should appear in the team list.
. You can now assign `Ronaldo` to `Arsenal` by typing the command `assign tm/Arsenal i/1` in the command box and pressing kbd:[Enter] to execute it.
. `Ronaldo` 's team field should now show Arsenal.
...
Furthermore, there are many ways you can customize and add to My Team Manager. Here are a few ways to start:
.. <<Editing>>
.. <<ChangeTheme>>
.. <<setTagColour>>
.. <<Privacy>>
