...
Code snippet from `Team.java` that shows the constructor of `Team`.
...
An additional `TeamName` field is also added to `Person` so that the user can quickly determine the team the player is in.
