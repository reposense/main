...
But before you start, there are two fundamental prerequisites you should about MTM:

=== Prerequisites

MTM needs to be written in Java 8 language and will be better written in an integrated development environment (IDE) such as IntelliJ.
...
Before you begin, ensure that you’ve done the following step on GitHub.

. Fork this repo, and clone the fork to your computer

Upon completion of the GitHub's portion on setting up, you can proceed to set up your project on IntelliJ.
...
The autocomplete feature works by handling the Tab key pressed event, searching for any commands with matching prefix and returns one if found. The command strings are stored a Trie data structure, named CommandTrie, for optimal search.
...
The trie is made up of node classes which only have three fields. The sibling field represents a character on the same level as the current one. The child represents a next possible letter. For example in the words edit and exit the node for 'd' would have a child 'i' and a sibling 'x'. 'x' would have a child 'i'.
...
----
Upon the pressing of the tab key, the command box calls attemptAutoComplete, in the commandTrie class. This method will traverse the trie using the content of the command box.
If the input has a character not present in the trie, then the attempt fails. If the trie parses through the input but reaches a node with a sibling option,
it will attempt to provide the user with possible commands for a given input. In this case, a dropdown box will appear in the UI that lists the possible commands for the user.

Upon finding a matching command, attemptAutoComplete returns it and replaces the text in the CommandLine. If the no matching command is found, the text is turned red.
...
** Pros: Easy to implement. Input is straightforward.
...
** Cons: Unattractice display of fields when it is unspecified.
* **Alternative 2:** Require users to input all fields
** Pros: Re-use current implementation and easier to clean up code
** Cons: Less flexibilty given to the user when adding a player into the application
...
[TIP]
====
The field `remark` is found in `Person`, however it is not be modified via `add` or `edit` commands. +
...
===== Aspect: How `remark` works
* **Alternative 1 (current choice):** Standalone command that add, edit, and delete remarks from `Person`
** Pros:
** Cons:
* **Alternative 2:** Usage of `edit` command to change or remove remark from `Person`
** Pros:
** Cons:
**
...
The 'create team' feature is facilitated by `CreateCommand` class. It allows users to create team into the application with the specified team namte. The team will be shown in

The command word for creating a team is `create`.
...
* In `SetCommandParser`, which implements the `Parser` interface, it parses the arguments inputted into the CLI, and checks whether
 ...
 ** Major - Creation of Team and related functions +
        ...
        viewing of teams,
        formation of team,
        indicate and set upcoming match

 ** Minor
    *** Remark command
    *** Enhanced add with optional field
