...
`Team` objects consist of `TeamName` object, which is used to uniquely identify the team, and `UniquePersonList` object which stores the information of `Person` objects which is identical to those found in `UniquePersonList` in `addressbook`.
...
Since `Person` stored in `Team` is referencing to the `Person` in `addressbook`, any changes made to `Person` will be immediately reflected. An additional `TeamName` field is also added to `Person` so that the user can quickly determine the team the player is in.

Code snippet from `AddCommand.java` that shows the reference to `Person` in `UniquePersonList` in `addressbnook`.
...
** Pros: Changes made to `Person` will be reflected on all references, easier lookup of person belonging in which team
** Cons: Checks need to be done thoroughly to ensure that code does not break when changes are made
